...
class RadiationFactoryTest(unittest.TestCase):
    ...
    def test_create_radiation(self):
        ...
        rad=fact_rad.create_for_single_electron(trajectory=traj_test, undulator=und_test)
        ...
        rad2=fact_rad.create_for_single_electron(trajectory=traj_test,undulator=und_test)
        ...
        B=und_test.create_magnetic_field_plane_undulator(Z=traj_test.z*codata.c,Y=0.0,harmonic_number=1)
        traj_test2=TrajectoryFactory(Nb_pts=1001, method=TRAJECTORY_METHOD_INTEGRATION).create_for_plane_undulator(
            undulator=und_test,B=B )
        rad3=fact_rad.create_for_single_electron(trajectory=traj_test2,undulator=und_test)

        err = rad2.difference_with(rad3)
        self.assertLessEqual(err.max(), 1.0e11)
